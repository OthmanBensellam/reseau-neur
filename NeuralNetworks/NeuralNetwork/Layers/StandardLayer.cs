...
namespace NeuralNetwork.Layers
{
    internal class StandardLayer : ILayer
    {
        ...
        public StandardLayer(Matrix<double> initialWeights, Matrix<double> initialBias, int batchSize, IActivator activator)// Matrix<double> VelocityB, Matrix<double> VelocityW
        {
        ...
        public void Propagate(Matrix<double> input )
        {
            ...
            int R_ = input.RowCount;
            Matrix<double> M = Weights_.Transpose() * input + Bias_;
            M.Map(Activator.Apply).CopyTo(Activation);
        }
        ...
        public void BackPropagate(Matrix<double> upstreamWeightedErrors)
        {
            ...
            int R_ = Activation.RowCount;
            Matrix<double> B = Activation.Map(Activator.ApplyDerivative) * upstreamWeightedErrors.Transpose();
            WeightedError = (Weights_ * B).Transpose();
            GradWeight_ = (Activation * B.Transpose()) / (double)BatchSize;
...
//the old version of the props for the batching:
/*
